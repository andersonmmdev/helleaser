...
export class Cli {
  ...
  async _processNewVersion(env) {
    ...
    const needGenerate =
      !this.argsParser.get("skip-version") && !this.argsParser.get("sv");
    ...
    if (needGenerate) {
      ...
      newVersion = newVersions[parseInt(option) - 1 || 0];
      if (!newVersion) {
      ...
      Logger.info("Skipping version update, will use current version.");
    }
